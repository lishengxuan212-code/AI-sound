...
use crate::tts_bridge::{
    retry_tts_playback, stop_tts_playback, synthesize_qwen_tts, RetryTtsPlaybackRequest, StopTtsPlaybackRequest,
    TtsInvokeRequest,
};
...
#[tauri::command(rename_all = "camelCase")]
pub async fn play_tts_audio(app: AppHandle, request: RetryTtsPlaybackRequest) -> Result<(), String> {
    retry_tts_playback(app, request).await
}

#[tauri::command(rename_all = "camelCase")]
pub async fn replay_tts_audio(app: AppHandle, request: RetryTtsPlaybackRequest) -> Result<(), String> {
    retry_tts_playback(app, request).await
}
